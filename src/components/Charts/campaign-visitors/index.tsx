...
export async function CampaignVisitors({ className }: { className?: string }) {
  ...
  return (
    <div className={cn('rounded-[10px] bg-white shadow-1 dark:bg-gray-dark dark:shadow-card', className)}>
      <div className='border-b border-stroke px-6 py-5.5 dark:border-dark-3'>
        <div className='flex justify-between'>
          <h2 className='mb-1.5 text-2xl font-bold text-dark dark:text-white'>Campaign Visitors</h2>
        ...
        <div className='flex justify-between'>
          ...
          <div className={cn('flex items-center gap-1.5', data.performance > 0 ? 'text-green' : 'text-red')}>
            <TrendingUpIcon className={`${data.performance > 0 ? '-rotate-6' : 'scale-y-[-1]'}`} />
