...
export async function TopChannels({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      <h2 className='mb-4 text-body-2xlg font-bold text-dark dark:text-white'>Top Channels</h2>
      ...
      <Table>
        ...
        <TableBody>
          {data.map((channel, i) => (
            <TableRow className='text-center text-base font-medium text-dark dark:text-white' key={channel.name + i}>
              ...
              <TableCell className='!text-right text-green-light-1'>${standardFormat(channel.revenues)}</TableCell>
